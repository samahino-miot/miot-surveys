  ...
  useEffect(() => {
    if (!loading && currentUser) {
      if (adminUser && adminUser.status === 'active') {
        navigate('/admin');
      } else if (adminUser && adminUser.status === 'pending') {
        setError('Your account is pending approval by a superadmin.');
      } else if (!adminUser) {
        setError('Your account is not registered as an admin. Please register first.');
      }
    }
    ...
    return (
      <div className="max-w-md mx-auto mt-12">
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200">
          ...
          {resetMessage && (
            <div className="mb-6 p-4 bg-teal-50 border border-teal-200 rounded-xl flex items-start gap-3 text-teal-700">
  ...
  return (
    <div className="max-w-md mx-auto mt-12">
      <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200">
        ...
        <p className="text-center text-slate-600 mb-8">Sign in with your Google account to access the dashboard.</p>
        ...
        <div className="mt-6 text-center text-sm text-slate-600">
          ...
          <Link to="/admin/register" className="text-teal-600 font-medium hover:underline">
            Register here
          </Link>
        </div>
      </div>
